...
def write_data_to_csv():
    with open("checking-staffs.csv", 'w', encoding='utf-8', newline='') as file:
          writer = csv.writer(file)
          for i in staffs_lst_default:
            writer.writerow(i)

# file_path = 'datas/staffs.csv'
def import_staffs_data(file_path):
  staffs_list_2 = []
  with open(file_path, 'r', encoding='utf-8', newline='') as file:
    reader = csv.reader(file)
    for row in reader:
       staffs_list_2.append(row)
  return staffs_list_2

# data = import_staffs_data(file_path)
# print(data)
